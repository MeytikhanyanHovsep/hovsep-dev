...
const Item = memo(function Item({ desc, img, name, link, proff }: Props) {
  return (
    <a
      href={link}
      target="_blank"
      className="group  max-w-137.5! [1400px]:max-w-175 relative min-h-full flex flex-col p-6 rounded-2xl  bg-[#0A0C10]/60 backdrop-blur-md border border-white/10 hover:border-emerald-500/30 hover:bg-[#0A0C10]/80 w-max max-sm:max-w-[90vw]! transition-all duration-300"
    >
      <HugeiconsIcon
        icon={Share05Icon}
      ...
      <div className="relative z-10 flex items-center  gap-3">
        ...
        <div className="h-auto mt-0.5 flex flex-col justify-center">
          <p className="text-sm leading-[100%] font-medium text-white">
            {name}
          </p>
          <p className="text-sm capitalize mt-0.5 text-slate-400">{proff}</p>
        </div>
        <div className="flex h-full group-hover:pr-7 mt-1.25 transition-all duration-300 ml-auto text-emerald-400 gap-0.5 ">
          {Array.from({ length: 5 }).map((_, i) => (
            <HugeiconsIcon
              key={i}
              className="fill-emerald-400 h-3 w-3"
              icon={StarIcon}
            />
          ))}
        </div>
